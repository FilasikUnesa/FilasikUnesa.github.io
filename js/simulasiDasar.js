	...
	*/
	...
	function slider (data){
		if (data.tipe == "H" || data.tipe == "h"){
			//slider H
			if (data.label != ""){
				teks(data.minS+" "+data.label, data.x+20, data.y+5, "14pt Calibri", "#000", "right");
				teks(data.maxS+" "+data.label, data.x+45+data.p, data.y+5, "14pt Calibri", "#000", "left");
			}
   ...
   function termometer(data){
		...
		for (i=0; i<data.label.length; i++)
        {
			teks(data.label[i], x-5, y+t+off*2-i*skala+5, "12pt Calibri", data.warnaGaris, "right");
